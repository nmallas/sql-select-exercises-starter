...
No parenthesis needed to get data in table format*/
...
SELECT city, (population_estimate_2018/1000000) AS 2018_pop_in_millions
FROM cities
WHERE state = 'Texas';
...
SELECT city, population_census_2010, population_estimate_2018
     FROM cities
...
--      SELECT airports.name, cities.city
--      FROM cities
--           INNER JOIN airports ON (cities.id = airports.city_id) WHERE cities.city = 'New York'
-- ) AS count;


SELECT COUNT(*) FROM airports
          INNER JOIN cities ON (cities.id = airports.city_id) WHERE cities.city = 'New York';
...
SELECT FAA_id, IATA_id, ICAO_id
     FROM airports WHERE name = 'Chicago O''Hare International Airport';
...
SELECT city, round(population_estimate_2018/1000000::decimal, 2) AS pop_in_mills
FROM cities
WHERE state = 'Texas';
